...
global_simulation_seconds = 60.0
...
def eval_CTRNN_genome(genome, config):
    ...
    for _ in range(global_runs_per_net):
        ...
        while not collision:
            ...
            inputs = [drone.pos[1][0], drone.vel[0][0], drone.vel[1][0], drone.theta_pos, drone.theta_vel] # inputs are laser lengths + state information
            ...
            target_z_dot = -min((0.1*drone.pos[1][0]**2+0.6),10)
            error_z_dot = abs(target_z_dot - drone.vel[1][0])
            error_theta = min(drone.theta_pos, abs(2*np.pi - drone.theta_pos))
            environment.fitness -= (error_z_dot + 10*error_theta)/10
        ...
        global_run_count += 1
        fitnesses.append(environment.fitness - total_t - drone.pos[1][0]*0)
...
def eval_NN_genome(genome, config):
    ...
    for _ in range(global_runs_per_net):
        ...
        # Find current obstacle Course
        if global_run_count%2 == 0:
            environment.resetEnv(course_1)
        else:
            environment.resetEnv(course_2)
        ...
        while not collision:
            # create list of inputs
            inputs = drone.laser_distances + [drone.pos[1][0], drone.vel[0][0], drone.vel[1][0], drone.theta_pos, drone.theta_vel] # inputs are laser lengths + state information
            action = net.activate(inputs)

            drone.update(action[0], action[1])
            ...
            collision = environment.collision or environment.touchdown
        global_run_count += 1
        fitnesses.append(environment.fitness + total_t - drone.pos[1][0]*10000)
